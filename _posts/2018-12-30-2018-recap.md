...
numbers! All representations of numbers have serious drawbacks:
 ...
 - fixed points are inaccurate (though this doesn't matter in practice, and I
   think fixed points are the winner here)
...
a point on the screen in 2d space, and every frame you draw the entire scene,
rather than just the parts that the eye can see,
but you want the visible part of the scene to be rendered differently.
...
checking if we encounter an opaque pixel. For pixels far from the eye, this
could involve many 100s of steps, each frame.

A key observation for improving
upon this is that most scenes have large areas of transparent pixels, and
sometimes also large areas of opaque pixels too. Consider a low-resolution
...
squares of pixels in the original image! We can just scan in a low resolution
image (which has fewer pixels, so the scan will take fewer steps), checking the
